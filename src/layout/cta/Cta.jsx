  ...
  return (
    <CTAStyleWrapper>
      <div className="container">
        <SectionTitle
          className="tda_title_section text-center"
          title="Interested in the project?"
        ></SectionTitle>
        <div className="title-emo">🤙</div>
        <div className="tda_v1_cta_content">
            ...
            <ul>
              {data?.map((item, i) => (
                <li key={i}>
                  <a href={item.url}>
                    {item.thumb ? (
                      <img src={item.thumb} alt="tda nft profiles" />
                    ) : (
                      item.icon
                    )}
                  </a>
            ...
            <div className="footer_stars">
              {particleShapes?.map((shape, i) => (
                <span key={i} className={`star_${i + 1}`}>
                  <img src={shape} alt="tda nft background particle" />
                </span>
              ))}
            </div>
          </div>
